...
function smallestMultiple(highestVal: number): number {
  let numberRange: number[] = [];

  for (let i = 2; i <= highestVal; i++) {
    numberRange.push(i);
  }

  for (let j = 0; j < numberRange.length - 1; j++) {
    for (let k = j + 1; k < numberRange.length; k++) {
  ...
  return numberRange.reduce((previousValue: number, currentValue: number): number => {
